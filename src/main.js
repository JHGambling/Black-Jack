...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (client) {
        ...
        client.casino.wallet.store.subscribe((wallet) => {
            balance = wallet.NetworthCents / 100;
            updateBalanceUI();
        });
    }
    ...
    function updateUI() {
        document.getElementById("player-cards").innerHTML = playerCards
            .map((card) => `<div>${card.value}${card.suit}</div>`)
            .join("");
    ...
    async function updateResult(message, playerWins) {
        document.getElementById("result").textContent = message;
        gameOver = true; // Set gameOver flag to true
        if (playerWins) {
            const winAmount = currentBet * 2;
            balance += winAmount;
            await processTransaction(winAmount, true);
    ...
    document.getElementById("place-bet").addEventListener("click", async () => {
        ...
        if (betValue > 0 && betValue <= balance) {
            currentBet = betValue;
            balance -= currentBet;
            await processTransaction(currentBet, false);
            updateBalanceUI();
            betInput.value = "";
            document.getElementById("result").textContent =
